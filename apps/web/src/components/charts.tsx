...
export function Chart({option,height=230,label}:{option:EChartsOption;height?:number;label:string}){
 ...
 useEffect(()=>{chart.current?.setOption({animation:!window.matchMedia('(prefers-reduced-motion: reduce)').matches,animationDuration:300,textStyle:{fontFamily:'Inter',fontSize:11,color:'#6f7d74'},grid:{top:32,left:44,right:20,bottom:32},tooltip:{trigger:'axis',backgroundColor:'#fff',borderColor:'#dce8da',textStyle:{color:'#203c32'}},...option},true);},[option]);
